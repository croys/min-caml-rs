...
pub fn ty_to_type_in_context(ctx: &Context, ty: &ty::Type) -> Type {
    fn to_type(ctx: &Context, ty: &ty::Type) -> LLVMTypeRef {
        match ty {
...
fn exp_to_llvm(
    ...
    fn to_val(c: &mut Constant) -> Value {
        let x = Value::from_ref(c.to_ref(), c.owned);
        c.release();
        x
    }
    ...
    match e {
        ...
        Nop => to_val(&mut Constant::int(&Type::int32_type_in_context(ctx), 0)),
        Set(x) => {
            to_val(&mut Constant::int(&Type::int32_type_in_context(ctx), *x))
        }
        SetL(ref l) =>
...
pub fn f(p: &closure::Prog) {
    ...
    let ctx = Context::new();
    let mut module = Module::create_with_name_in_context("mincaml_main", &ctx);
